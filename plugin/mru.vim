...
"       let MRU_Exclude_Files = '.*\fav\.list'
" 
...
function! s:MRU_LoadList()
	...
	" call s:MRU_Warn_Msg("MRU_LoadList: begin ") 
...
function! s:MRU_LoadFavorites()
    " Read the list from the MRU file.
	" call s:MRU_Warn_Msg("MRU_LoadFavorites: begin ") 
    if filereadable(g:MRU_FavFile)
		"call s:MRU_Warn_Msg("MRU_LoadFavorites: if filereadable(g:MRU_FavFile") 
        let s:MRU_files = readfile(g:MRU_FavFile)
        if s:MRU_files[0] =~# '^" Favorite files in Vim'
			"call s:MRU_Warn_Msg("MRU_LoadFavorites: if s:MRU_files[0] =~# '^\" Favorite files in Vim'") 
            " Generated by the previous version of the MRU plugin. Ignore the
            " list
            let s:MRU_files = []
        elseif s:MRU_files[0] =~# '^#'
...
function! s:MRU_Edit_File(filename)
    " call s:MRU_Warn_Msg("Edit File-in-function")
...
function! s:MRU_Window_Edit_File(win_opt)
    ...
    let labelIdx = stridx(fname, '|')
    if labelIdx > 0
        let fname = fname[labelIdx + 1:]
    endif
...
function! s:MRU_Open_Window(...)
    ...
    " Load the latest MRU file list
    if a:0 == 1 && a:1 == '-f'
		" call s:MRU_Warn_Msg("MRU_open_Window, call MRU_LoadFavorites ") 
        call s:MRU_LoadFavorites()
        let bname = '__MRU_Files__'
        "let bname = '__Favorite_Files__'
    else
		" call s:MRU_Warn_Msg("MRU_open_Window, call MRU_LoadFiles ") 
        call s:MRU_LoadList()
        let bname = '__MRU_Files__'
    endif
    ...
    nnoremap <buffer> <silent> <CR> :call <SID>MRU_Window_Edit_File('useopen')<CR>
    nnoremap <buffer> <silent> o :call <SID>MRU_Window_Edit_File('newwin')<CR>
    nnoremap <buffer> <silent> t :call <SID>MRU_Window_Edit_File('newtab')<CR>
    nnoremap <buffer> <silent> u :MRU<CR>
    nnoremap <buffer> <silent> <2-LeftMouse> :call <SID>MRU_Window_Edit_File('useopen')<CR>
    nnoremap <buffer> <silent> q :close<CR>
    nnoremap <buffer> <silent> <Esc> :close<CR>
"     let i = 1
"     while i <= MRU_Max_Entries
"         if i > 9
"             let i = nr2char(char2nr('a') + i - 9)
"         endif
"         nnoremap <buffer> <silent> i :call <SID>MRU_Window_Edit_File(i)<CR>
"     endwhile
...
function! s:MRU_Cmd(pat)
    if a:pat == ''
        ...
        return
    elseif a:pat == '-f'
        " Load the favorites window
        call s:MRU_Open_Window('-f')
        return
    endif
    ...
    if len(m) == 1
        " call s:MRU_Warn_Msg("Edit File-0")
    ...
    if len(m) == 0
        ...
        call s:MRU_Warn_Msg("MRU file list doesn't contain files containing " . a:pat)
        return
    endif

    if len(m) == 1
        " call s:MRU_Warn_Msg("Edit File-1")
        call s:MRU_Edit_File(m[0])
        return
    endif
